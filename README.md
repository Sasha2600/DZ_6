...
5. **Алертинг** — `check_error_alert`: если доля прогонов с эскалацией выше
   `ERROR_ALERT_THRESHOLD` (0.2), в конце `--demo` и в `--report` печатается
   строка `[АЛЕРТ] … — проверьте LLM и конфигурацию`.
...
«Успешность 50%» и `[АЛЕРТ]` в демо — ожидаемо: прогон 2 (refuse) — нормальный
отказ, прогон 3 (high_risk) — запланированная эскалация, и 1/4 = 25% > порога
20%; по-настоящему «провалились» 0 прогонов. На большом логе алерт — сигнал
проверять LLM и конфигурацию.
...
- **Стоимость и алертинг.** `cost_rub` (рубли) считается по тарифам за
  ...
  запись лога, строку метрик, сводку и отчёт; если доля эскалаций выше
  `ERROR_ALERT_THRESHOLD` (20%), в конце `--demo` и в `--report` печатается
  `[АЛЕРТ] … — проверьте LLM и конфигурацию`.
...
`.venv/bin/python agent.py --selftest` — 12 проверок, LLM-сервер, Qdrant и сеть
...
12. алерт: доля эскалаций выше порога 20% → текст алерта, ровно 20% и ниже —
