...
1. **Метрики** — `RunMetrics` на каждый прогон: успех (outcome), длительность,
   LLM-вызовы, токены (prompt/completion/total), стоимость в рублях
   (`cost_rub`: входные и выходные токены по отдельным тарифам за 1M токенов
   `LLM_COST_INPUT_PER_1M_RUB` / `LLM_COST_OUTPUT_PER_1M_RUB`), ретраи,
   нарушение бюджета.
2. **Ограничение** — `BudgetGuard` (преодохранитель): не более
 ...
 3. **Проверка ответа** — новое состояние `check_answer` (13-е в графе, между
    ...
    **и получены текущим поиском** (top-k — защита от «галлюцинированных» и
    «чужих» источников). При провале — обычные ретраи, финальная причина
    эскалации — `answer_check_failed`.
4. **Лог выполнения** — `logs/runs.jsonl` (JSONL, одна строка на прогон:
   ts, query, outcome, причина, trace, время, вызовы, токены, `cost_rub`,
   ретраи, memory_saved, sources) + сводная таблица метрик в `--demo` +
   агрегаты в `--report`. Ротация: при `RUNS_LOG_MAX_RECORDS` (1000) записях
   файл уходит в архив `runs.jsonl.1` (один архив), лог не растёт бесконечно.
 5. **Алертинг** — `check_error_alert`: если доля прогонов с *технической*
    эскалацией выше `ERROR_ALERT_THRESHOLD` (0.2), в конце `--demo` и в
    `--report` печатается строка `[АЛЕРТ] …`. Safety-эскалация `high_risk` —
    штатное безопасное поведение агента — ошибкой не считается.
 6. **Оценка качества** — gold set `context/eval.json` (вопросы + ожидаемые
    факты ответа + ожидаемый источник) и режим `--eval`: измеряет именно
    качество ответа, а не только успешное завершение сценария.
...
agent.py              # движок графа, сценарий (13 состояний), Qdrant-память,
                       # контрольный слой (метрики/бюджет/retry/лог/eval), CLI, selftest
...
# самодиагностика без LLM, Qdrant и сети (14 проверок)
...
# Нужны LLM и Qdrant; память (memory/qa.json) и лог (logs/runs.jsonl) не трогает.
...
После завершения курса сертификат можно получить в личном кабинете, обычно в течение 5 рабочих дней.

[источники: doc-certificate]
Источники: doc-certificate, doc-enroll, doc-mentor
Сохранено в память (qa.json).
Метрики: 86.21с | LLM-вызовов: 3 | токены: 2119 (prompt=895, completion=1224) | стоимость: 0.0869₽ | ретраев: 0
...
Метрики: 33.07с | LLM-вызовов: 1 | токены: 705 (prompt=209, completion=496) | стоимость: 0.0329₽ | ретраев: 0
...
Причина: high_risk
Метрики: 30.05с | LLM-вызовов: 1 | токены: 644 (prompt=215, completion=429) | стоимость: 0.0290₽ | ретраев: 0
...
1   answered              3     2119    86.21с        0     0.0869₽  happy path: полный путь + сохранение в память
2   refused               1      705    33.07с        0     0.0329₽  нет контекста → ветка refuse
3   escalated             1      644    30.05с        0     0.0290₽  high_risk
4   answered_cached       0        0     0.00с        0     0.0000₽  повтор вопроса 1 → хит в памяти
Успешность: 2/4 (50.0%) | отказов: 1 | safety-эскалаций: 1 | тех. ошибок: 0
Среднее время: 37.33с | суммарные токены: 3468 | стоимость: 0.1487₽ | ретраев: 0
```

`[АЛЕРТ]` в демо нет: единственная эскалация — `high_risk`, это штатная
safety-эскалация, а не техническая ошибка.
...
результат          прогонов     доля   среднее время    токенов    стоимость
answered                  1    25.0%          86.21с       2119      0.0869₽
answered_cached           1    25.0%           0.00с          0      0.0000₽
refused                   1    25.0%          33.07с        705      0.0329₽
escalated                 1    25.0%          30.05с        644      0.0290₽
Успешность: 2/4 (50.0%) | safety-эскалаций: 1 | тех. ошибок: 0
Суммарно: время 149.33с, токенов 3468, стоимость 0.1487₽, ретраев 0, эскалаций по бюджету 0
```

«Успешность 50%» в демо — ожидаемо: прогон 2 (refuse) — нормальный отказ,
прогон 3 (high_risk) — запланированная safety-эскалация, не ошибка;
по-настоящему «провалились» 0 прогонов. На большом логе алерт (только по
техническим эскалациям) — сигнал проверять LLM и конфигурацию.
...
AgentResult + RunMetrics → RunLog (logs/runs.jsonl, JSONL, append,
                  ...
                  + check_error_alert (доля *технических* эскалаций > порога → [АЛЕРТ];
                                        safety high_risk не учитывается)
```

`--eval` — отдельный путь: gold set `context/eval.json` прогоняется в изолированной
среде (свежая tmp-память), по кейсам печатается итог, доля фактов, источник и
стоимость; в `runs.jsonl` и `memory/qa.json` ничего не пишется.
...
- **Две проверки ответа.** `check_answer` (новый, детерминированный, без LLM):
  ...
  в БЗ и получены текущим поиском (модель цитирует только то, что видела);
  затем LLM-валидатор grounding-фактов. Обе используют общий ретрай-механизм,
  но причины эскалации разные: `answer_check_failed` /
...
- **Лог — часть сдачи, с ротацией.** Каждый прогон (одиночный, интерактив,
  демо) дописывает одну JSON-строку в `logs/runs.jsonl`; при переполнении
  (`RUNS_LOG_MAX_RECORDS`, 1000) файл уходит в архив `runs.jsonl.1` (хранится
  один архив) — лог не растёт бесконечно. Selftest пишет только в tmp-каталог.
- **Стоимость и алертинг.** `cost_rub` (рубли) считается по тарифам за
  1 000 000 токенов, входные и выходные токены тарифицируются отдельно
  (`LLM_COST_INPUT_PER_1M_RUB` / `LLM_COST_OUTPUT_PER_1M_RUB`), и попадает в
  запись лога, строку метрик, сводку и отчёт; алерт срабатывает, если доля
  *технических* эскалаций выше `ERROR_ALERT_THRESHOLD` (20%) — safety-эскалация
  `high_risk` (штатное безопасное поведение) ошибкой не считается.
- **Gold set — качество, а не только успех.** `outcome` говорит, что сценарий
  ...
  прогоняет их и печатает долю попавших фактов — так измеряется качество
  ответов. Факты обязаны оставаться подстроками документов БЗ (проверяется
  selftest).
...
| `LLM_COST_INPUT_PER_1M_RUB` | `15` | тариф за 1M входных (prompt) токенов (₽) для метрики `cost_rub` |
| `LLM_COST_OUTPUT_PER_1M_RUB` | `60` | тариф за 1M выходных (completion) токенов (₽) для метрики `cost_rub` |
| `ERROR_ALERT_THRESHOLD` | `0.2` | порог доли *технических* эскалаций: выше — `[АЛЕРТ]` в `--demo`/`--report` (safety `high_risk` не учитывается) |
...
`.venv/bin/python agent.py --selftest` — 14 проверок, LLM-сервер, Qdrant и сеть
...
9. retry: режим `flaky` (первые 2 вызова — имитация сетевой ошибки) →
 ...
 10. check_answer: режим `badformat` (ответ с несуществующим `doc-fake` в
     ...
     не вызывается; плюс retrieval-область: id из БЗ, но не из топ-k текущего
     поиска → тоже отклоняется;
 11. ротация лога: `runs.jsonl` не растёт за `max_records`, переполненный файл
     целиком в архив `<файл>.1` (архив один, не накапливается),
     `max_records=0` — ротация выключена;
 12. алерт: учитываются только *технические* эскалации — 25–50% `high_risk`
     алерта не даёт, одна `step_error` из 4 прогонов — даёт, ровно 20% и ниже
     — тишина, пустой лог — алерта нет;
 13. классификация эскалаций: `high_risk` — safety (штатное поведение),
     `step_error:*`/`budget_exceeded`/`max_steps`/провалы валидаций — technical;
 14. gold set: `context/eval.json` валиден (каждый факт — подстрока текста
